...
df_std.tr1 <- prune(df_std.tr, cp=0.00991235)  #approximately the best size pruned tree
...
##Answer: 
control_best <- rpart.control(minbucket = 5, cp = 0.00991235, maxsurrogate = 0, usesurrogate = 0)
...
##Answer: The neural network with the smallest misclassification rate has $\lambda = $ and number of hidden nodes as . The misclassification rate is .

##CV function for classification
CVfunc_nnet_clf <- function(data, lam_seq, num_hidnode_seq,Nrep,K,y) {
  n=nrow(data)
  n.models = n.lam*n.num_hidnode #number of different models to fit
  yhat=matrix(0,n,n.models)
...
par_best_crude <- CVfunc_nnet_clf(FGL1[,c(1:10)], lam_seq, num_hidnode_seq,Nrep,K,y)
proc.time() - ptm
...
num_hidnode_seq = seq(22,30,2)  

par_best <- CVfunc_nnet_clf(FGL1[,c(1:10)], lam_seq, num_hidnode_seq,Nrep,K,y)
proc.time() - ptm
